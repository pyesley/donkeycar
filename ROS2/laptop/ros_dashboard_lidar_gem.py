...
class RobotControlNode(Node, QtCore.QObject):
    ...
    def __init__(self):
        ...
        # On startup: try to stop lidar motor explicitly so we start in a known state
    ...
    # ---------- LiDAR control ----------
    def control_lidar(self, enable: bool):
        """
        Call the ROS 2 service to start (True) or stop (False) the LiDAR.
        Stopping the LiDAR via the C++ node will also stop the motor.
        """
        if not self.lidar_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn("LiDAR service not available.")
            return False

        req = SetBool.Request()
        req.data = bool(enable)

        # Call async so we don't block the ROS thread
        future = self.lidar_client.call_async(req)

        # We can define a local callback to log the result
        def _done(f):
            try:
                resp = f.result()
                state_str = "started" if enable else "stopped"
                if resp.success:
                    self.get_logger().info(f"LiDAR {state_str} successfully: {resp.message}")
                else:
                    self.get_logger().error(f"LiDAR {state_str} failed: {resp.message}")
    ...
    def shutdown(self):
        ...
        # Ensure LiDAR is stopped on exit
        self.control_lidar(False)
        time.sleep(0.1)
        self.destroy_node()
...
class RobotControlWindow(QtWidgets.QMainWindow):
    def __init__(self, ros_node: RobotControlNode):
        ...
        self.setWindowTitle("Robot Control Dashboard (LiDAR Manager)")
        ...
        self.is_lidar_display_active = False  # Controls if we draw data
        ...
        self.btn_start.clicked.connect(self._on_lidar_start)
        ctrl_row.addWidget(self.btn_start)

        # Stop Button
        self.btn_stop = QtWidgets.QPushButton("Stop Scan")
        self.btn_stop.setStyleSheet("background-color: #cd5c5c; color: white; font-weight: bold; padding: 5px;")
        self.btn_stop.clicked.connect(self._on_lidar_stop)
        ...
        # Status label
        self.status_label = QtWidgets.QLabel("LiDAR Stopped")
        ...
        self._setup_plot(self.accel_plot, "Linear Accel", "m/s²")
        ...
        self._setup_plot(self.gyro_plot, "Angular Vel", "rad/s")
        ...
        self.enc_plot = pg.PlotWidget();
        self._setup_plot(self.enc_plot, "Encoder Count", "ticks")
        ...
        self.lidar_timer = QtCore.QTimer(self);
        self.lidar_timer.timeout.connect(self._refresh_lidar)
        self.lidar_timer.start(int(1000.0 / LIDAR_DRAW_FPS))

        self.setFocusPolicy(QtCore.Qt.StrongFocus)

        # Reduce OpenCV thread fan-out
        try:
            cv2.setNumThreads(1)
        except Exception:
            pass
    ...
    def _on_image(self, cv_image):
        ...
        try:
            ...
            self.ros_node.get_logger().error(f"Error updating camera feed: {e}")
    ...
    def _on_lidar_msg(self, msg: LaserScan):
        # We just store the message. The timer handles drawing.
        self.last_scan_msg = msg

    # ----- NEW LIDAR BUTTON HANDLERS -----
    def _on_lidar_start(self):
        # 1. Update UI state
        self.is_lidar_display_active = True
        self.btn_start.setEnabled(False)
        self.btn_stop.setEnabled(True)
        self.status_label.setText("Starting LiDAR...")

        # 2. Call ROS Service
        self.ros_node.control_lidar(True)

    def _on_lidar_stop(self):
        # 1. Update UI state
        self.is_lidar_display_active = False
        self.btn_start.setEnabled(True)
        self.btn_stop.setEnabled(False)
        self.status_label.setText("LiDAR Stopped")

        # 2. Clear the plot immediately
        self.lidar_scatter.setData([], [])

        # 3. Call ROS Service
        self.ros_node.control_lidar(False)

    # ----- Lidar drawing (timer) -----
    def _refresh_lidar(self):
        # Only draw if the user has pressed Start
        if not self.is_lidar_display_active:
            return
        ...
        try:
            ...
            if not np.any(mask):
                self.status_label.setText("Scanning... (No valid points)")
                return
            ...
            # Decimate if too many points to keep UI snappy
            xs, ys = self._decimate_xy(xs, ys)

            self.lidar_scatter.setData(xs, ys)
            self.status_label.setText(f"Scanning... ({len(xs)} points)")

        except Exception as e:
            self.ros_node.get_logger().error(f"Lidar draw error: {e}")
    ...
    def keyPressEvent(self, event):
        ...
        # Keyboard shortcuts for LiDAR
        if key == QtCore.Qt.Key_O:
            self._on_lidar_start();
            return
