...
class IMU:
    ...
    def __init__(self, addr=0x68, poll_delay=0.0166, sensor=SENSOR_MPU6050, dlp_setting=DLP_SETTING_DISABLED):
        ...
        elif self.sensortype == SENSOR_ARDUINO:
            from donkeycar.parts.arduinoSerial import ARDUINONanoSerial
            self.sensor = ARDUINONanoSerial()
            print('IMU:Started Arduino Nano Serial')
        else:
    ...
    def poll(self):
        try:
            if self.sensortype == SENSOR_MPU6050:
                ...
                self.accel, self.gyro, self.mag = self.sensor.request_imu()
            else:
